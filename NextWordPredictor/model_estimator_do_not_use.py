...
from tensorflow.contrib.feature_column import sequence_categorical_column_with_hash_bucket, sequence_categorical_column_with_vocabulary_list
...
with open(path) as f: # Use file to refer to the file object
    ...
    labels = np.zeros(len(text_slices))
...
token_sequence = sequence_categorical_column_with_hash_bucket(
    key="text", hash_bucket_size=80, dtype=tf.string)
# what does this even mean???
token_emb = embedding_column(categorical_column=token_sequence, dimension=80)


#token_sequence = sequence_categorical_column_with_vocabulary_list(
#    key="text", vocabulary_list=chars, dtype=tf.string)
# what does this even mean???
#token_emb = embedding_column(categorical_column=token_sequence, dimension=80)
    
print([token for token in token_emb])
